...
describe('terpeneHelpers', () => {
  ...
  describe('parseConcentration', () => {
    ...
    it('assigns High label for ≥75th percentile', () => {
  ...
  describe('getSourceIcon', () => {
    it('returns 🍋 for "Lemon peel"', () => {
      expect(getSourceIcon('Lemon peel')).toBe('🍋');
    });

    it('returns 🍊 for "Orange rind"', () => {
      expect(getSourceIcon('Orange rind')).toBe('🍊');
    });

    it('returns 🌿 for "Lavender"', () => {
      expect(getSourceIcon('Lavender')).toBe('🌿');
    });

    it('returns 🌲 for "Pine needles"', () => {
      expect(getSourceIcon('Pine needles')).toBe('🌲');
    });

    it('returns 🌶️ for "Black pepper"', () => {
      expect(getSourceIcon('Black pepper')).toBe('🌶️');
    });

    it('returns 🍺 for "Hops"', () => {
      expect(getSourceIcon('Hops')).toBe('🍺');
    });

    it('returns 🌿 for unknown sources', () => {
      expect(getSourceIcon('Unknown source')).toBe('🌿');
    });
  ...
  describe('copyToClipboard', () => {
    it('has the correct function signature', () => {
      ...
      expect(copyToClipboard.length).toBe(3); // text, onSuccess, onError
    });
    ...
    it('handles empty text gracefully', async () => {
      // Should not throw with empty text
