...
test.describe('Effect Translation and Category Mapping', () => {
  ...
  test('should preserve effect category mapping while translating display names', async ({ page }) => {
    ...
    const effectChips = page.locator('role=button').filter({ has: page.locator('text=Couch-lock') }).or(
      page.locator('role=button').filter({ has: page.locator('text=Couchfesselung') })
    );
